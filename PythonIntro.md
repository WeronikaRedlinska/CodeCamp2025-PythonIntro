...
👉 Teraz przejdźmy na większą skalę i wypróbujmy kilka kodów...
*Pewnie już o tym wiesz, ale upewnij się, że najpierw usuniesz stary kod i dodasz cały nowy kod do pliku `main.py`.*

```python
number = input("Podaj liczbę: ")
group = input("Podaj rzeczownik zbiorowy dla grupy rzeczy: ")
thing = input("Podaj nazwę dziwnej lub zwariowanej rzeczy: ")
print("Nie, nie sądzę, że", number, "to", group, "z", thing,". To po prostu dziwne.")
```

### Możesz łączyć ze sobą dowolnie wiele elementów, w dowolnej kolejności, pod warunkiem, że są one oddzielone przecinkami!

# Popularne Błędy

*Pierwsze, usuń jakikolwiek inny kod w pliku `main.py`. Skopiuj każdy fragment kodu poniżej do pliku `main.py`, klikając na ikonę kopiowania w prawym górnym rogu każdego pola z kodem. Następnie uruchom kod, aby zobaczyć, jakie błędy występują. Napraw błędy i uruchom kod ponownie, aż nie będzie błędów. Kliknij na '👀 Answer', aby porównać swój kod z kodem poprawnym.*

## Błędna Składnia

👉 Skopiuj poniższy kod, uruchom go i zobacz, co się dzieje.
...
```
Co oznacza ten błąd? Jak można go naprawić?
...
<details><summary> 👀 Odpowiedź </summary>
- Łatwo popełnić błąd, zapominając o jednym lub kilku przecinkach.
- Pamiętaj, że potrzebujesz `,` między każdym różnym obiektem, który próbujesz wyświetlić
- Bez `,` komputer się myli i pokazuje błąd.
</details>

## Twój kod może działać... ale wyglądać dziwnie
👉 Ten kod faktycznie by działał, ale...
...
print("yourName, thinks it is, whatYear")
```
daje taki wynik. Co jest dziwnego w tym kodzie?
```
Name: Mark
...
<details><summary>👀 Odpowiedź</summary>
  ...
  - Innym częstym błędem jest owinięcie całego kodu w **cudzysłów** `(".., .., .. ")`.
  - Ten kod *faktycznie działa*, ale nie robi tego, co chcesz.
  - Wszystko w `(".., .., .. ")` jest wydrukowane dosłownie.
  - Ponieważ nazwy zmiennych są również wewnątrz cudzysłowów `(".., .., .. ")`, jest dosłownie drukowana ich nazwa, a nie ich zawartość.
    - Jedynie treści dosłowne powinny być w cudzysłowach `print(varName, "treść dosłowna", varName2)`.
</details>
...
```python
print("=== Twój Generator Piosenek ===")
print("""Zostaniesz poproszony/a o wiele pytań
potem stworzymy dla ciebie niesamowitą
piosenkę, całkowicie wolną od praw autorskich 😭""")
print()
person = input("Podaj imię osoby znanej z czegoś: ")
thing = input("Podaj coś co zrobiła: ")
place = input("Podaj miejsce, które lubisz: ")
rhyme = input("Podaj czasownik, który rymuje się z imieniem tej osoby: ")
print()
print("Była sobie osoba o imieniu", name)
print("Która zrobiła coś fajnego jak", thing, "w cudownym", place, "gdzie znajdziesz mnie", rhyme)
...
print()
print("Była sobie osoba o imieniu", person, "Która zrobiła coś fajnego jak", thing, "w cudownym", place,  "gdzie znajdziesz mnie", rhyme)
```
</details>
